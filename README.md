...
back to the dotnet API. The dotnet API is available at `http://localhost:5001/` and ALSO set up to proxy non-api requests to the webpack server. Both should work, although the convention for dotnet is to go directly through the dotnet app @ `http://localhost:5001`.
...
``` bash
curl -H "Content-Type: application/json" -X POST --data '{"name": "Frank Zappa", "emailAddress": "frank@zappamusic.org"}' http://localhost:5001/api/petowners/
{
   "id" : 76,
   "name" : "Frank Zappa",
...
``` bash
$ curl -H "Content-Type: application/json" -X POST --data '{"name": "Fido", "breed": "bulldog", "color": "Black", "petOwnerid": 76}' http://localhost:5001/api/pet
{
   "id" : 40,
   "petOwner" : {
      "petCount" : 1,
...
``` bash
$ curl -H "Content-Type: application/json" -X PUT --data '' http://localhost:5001/api/pets/40/checkin
{
   "name" : "Fido",
   "petOwner" : null,
   "id" : 40,
...
``` bash
$ curl -H "Content-Type: application/json" -X PUT --data '' http://localhost:5001/api/pets/40/checkout
{
   "checkedInAt" : null,
   "id" : 40,
   "name" : "Fido",
...
``` bash
$ curl -H "Content-Type: application/json" -X DELETE -i http://localhost:5001/api/pets/44
...
```
$ curl -H "Content-Type: application/json" -X DELETE -i http://localhost:5001/api/petOwners/76
HTTP/1.1 204 No Content
